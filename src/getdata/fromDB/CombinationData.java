		...
		for (int y = 1; y < 4; y++) {
			...
			for (int i = 0; i < awsInfo.length; i++) {
				...
				if (forecastDataArray[i].getNxNy().equals(awsInfo[i].getNx() + "" + awsInfo[i].getNy())) {
					...
					List<SensorData> PredataList = GetPastData.GetPastData(awsInfo[i].getGetUrl(), awsInfo[i].getPre());
					...
					List<SensorData> TmidataList = GetPastData.GetPastData(awsInfo[i].getGetUrl(),
							awsInfo[i].getTmin());
					List<SensorData> HumdataList = GetPastData.GetPastData(awsInfo[i].getGetUrl(), awsInfo[i].getHum());
					List<SensorData> TavdataList = GetPastData.GetPastData(awsInfo[i].getGetUrl(), awsInfo[i].getTav());
